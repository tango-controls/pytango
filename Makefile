...
# $ conda create -n [YourEnvName] -c tango-controls -c conda-forge python=[choose 2 or 3] tango pybind11 numpy gxx_linux-64 gevent pytest
...
ifndef prefix
ifdef user
PY_DIR=$(shell $(PY_EXC) -c "import sys, os; sys.stdout.write(os.path.split(os.path.join(os.path.dirname(os.__file__)))[1])")
prefix=$(HOME)/.local/lib/$(PY_DIR)/site-packages
else
PY_DIR=$(shell $(PY_EXC) -c "import sys, os; sys.stdout.write(os.path.join(os.path.dirname(os.__file__)))")
prefix=$(PY_DIR)/site-packages
endif
endif
...
CFLAGS += -std=c++14 -fvisibility=hidden -g $(OPTIMIZE_CC)
CFLAGS += -I$(PYTHON_INCLUDE_DIR)
CFLAGS += -I$(CONDA_PREFIX)/include/tango -Wno-deprecated
CFLAGS += -I$(CONDA_PREFIX)/include
CFLAGS += -I$(PY_DIR)/site-packages/numpy/core/include
CFLAGS += -Iext
LNFLAGS = -L$(CONDA_PREFIX)/lib -ltango
