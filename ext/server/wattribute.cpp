...
/// @{ 
	...
	if (ptr == NULL) {
        ...
        long length = att.get_write_value_length();
        for (long l = 0; l < length; ++l) {
            seq.append(from_char_to_str2(ptr[l]));
        }
    }
	...
	if (ptr == NULL) {
	    *obj = boost::python::object();
        ...
        boost::python::list o;
        for (long l = 0; l < length; ++l) {
            o.append(from_char_to_str2(ptr[l]));
	}
    }
	...
	if (buffer == NULL) {
	    *obj = boost::python::object();
        ...
        if (att.get_data_format() == Tango::SPECTRUM) {
            for (size_t x=0; x<dim_x; ++x) {
                result.append(from_char_to_str2(buffer[x]));
            }
        } else {
            for (size_t y=0; y<dim_y; ++y) {
                boost::python::list row;
                for (size_t x=0; x<dim_x; ++x) {
                    row.append(from_char_to_str2(buffer[x + y*dim_x]));
                }
                result.append(row);
            }
        }
